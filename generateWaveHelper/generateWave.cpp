...
//FM信号を生成するヘルパー関数, 搬送波もこの関数で生成してくれるおまけ付き
//[in] fc = キャリア周波数,[in] b = 変調指数,[in] input = 情報信号,
Graph GenerateWave::generateFmWave(const double fc, const double b, const Graph input)
{
   for(unsigned i = 0; i < input.x.size(); ++i)
   {
      t_[i] = input.x[i];
      v_[i] = std::sin((2 * pi * fc * input.x[i]) + (b * input.y[i]));
   }
   return { t_,v_ };
}
...
Graph GenerateWave::applyIntegral()
{
   for (auto i = 0; i < v_.size(); ++i)
   {
      if (checkPlusNumber<int>(i - 1))
      {
         v_[i - 1] += v_[i];
      }else
      {
